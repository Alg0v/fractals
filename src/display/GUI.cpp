...
namespace GUI {
    ...
    int (*coloring) (const std::pair<uint8_t, double>& data) = color::algov;
    ...
    int render_t(void * data){
        ...
        for(int y = rRect->y; y < rRect->h + rRect->y; y++)
            for (int x = rRect->x; x < rRect->w + rRect->x; x++) {
                pixels_buffer[x + y * window_rect.w] = coloring(algorithm(math::Complex((double)(((double)x/(double)_zoom) + offsetX),(double)(((double)y/(double)_zoom) + offsetY)), 64));
            }
            ...
            while(SDL_PollEvent(&event)){
                switch(event.type){
                    ...
                    case SDL_MOUSEMOTION:
                        if(mousedown){
                            SDL_Rect frame_rect = { -(mouse_start.x - event.motion.x), -(mouse_start.y - event.motion.y), window_rect.w, window_rect.h};
                            SDL_SetRenderDrawColor(renderer, 0, 255, 0, 255);
                            SDL_RenderClear(renderer);
                            SDL_RenderCopyEx(renderer, frame, nullptr, &frame_rect, 0, nullptr, (SDL_RendererFlip) (SDL_FLIP_HORIZONTAL | SDL_FLIP_VERTICAL));
                    ...
                    case SDL_MOUSEBUTTONUP:
                        if(mousedown){
                            ...
                            for(int y = math::max(deltaY); y < window_rect.h && y - deltaY < window_rect.h; y++)
                                for(int x = math::max(deltaX); x < window_rect.w && x - deltaX < window_rect.w; x++){
                    ...
                    case SDL_MOUSEWHEEL:
                        ...
                        if(event.wheel.y > 0){
                            ...
                            SDL_RenderCopyEx(renderer, frame, &r, NULL, 0, NULL, (SDL_RendererFlip) (SDL_FLIP_HORIZONTAL | SDL_FLIP_VERTICAL));
                        } else{
                            ...
                            SDL_SetRenderDrawColor(renderer, 0, 255, 0, 255);
                            SDL_RenderClear(renderer);
                            SDL_RenderCopyEx(renderer, frame, NULL, &r, 0, NULL, (SDL_RendererFlip) (SDL_FLIP_HORIZONTAL | SDL_FLIP_VERTICAL));
                        }
                        SDL_RenderPresent(renderer);
                        break;
